...
<div class="user-now card-item flex-fill mb-3 row">
  ...
  <div class="row-fill">
    ...
    <span class="userlist-memos button d-md-flex p-2 mr-2" hidden aria-hidden="true">
    ...
    <span class="randomuser-memos button d-md-flex p-2 mr-2" hidden aria-hidden="true">
    ...
    <span class="gobbs-memos button d-md-flex p-2 mr-2" hidden aria-hidden="true">
...
document.addEventListener("DOMContentLoaded", () => {
  if (typeof memosOwner === "undefined" || !memosOwner.link) {
    console.error("Memos owner is not configured.");
    return;
  }

  // 只使用当前站点配置的个人 Memos，不再加载公共订阅列表。
  memoList = [memosOwner];
